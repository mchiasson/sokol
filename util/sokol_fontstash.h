...
static const uint8_t _sfons_fs_bytecode_wgpu[1000] = {
...
static int _sfons_render_create(void* user_ptr, int width, int height) {
    ...
    if (sfons->shd.id == SG_INVALID_ID) {
        ...
        #elif defined(SOKOL_WGPU)
            shd_desc.vs.bytecode = SG_RANGE(_sfons_vs_bytecode_wgpu);
            shd_desc.fs.bytecode = SG_RANGE(_sfons_fs_bytecode_wgpu);
        #else
